...
# IMPORTANTE: Asegúrate de tener core/updater.py creado
...
class MainDashboard(ctk.CTk):
    def __init__(self):
        ...
        # 1. Cargar Configuración Local (Versión)
        self.local_version = "1.0.0 Guardian Edition" # Valor por defecto
        try:
            with open('config.json', 'r') as f:
                config = json.load(f)
                self.local_version = config.get("version", "1.0.0 Guardian Edition")
        except Exception:
            print("Advertencia: No se pudo leer config.json")
        ...
        # 3. Configurar Updater (CORRECCIÓN CRÍTICA AQUÍ)
        self.repo_json_url = "https://raw.githubusercontent.com/sao2139/SAO-Antivirus-Master/refs/heads/main/version.json"
        
        # NO iniciamos el thread aquí directamente.
        # Usamos 'after' para esperar 3 segundos (3000ms) a que la ventana cargue.
        self.after(3000, self.init_update_thread)
        ...
        # Iniciar bucle de refresco de UI
        self.after(1000, self.update_system_status)

    def init_update_thread(self):
        """Inicia el hilo de actualización de forma segura tras cargar la UI."""
    ...
    def setup_sidebar(self):
        ...
        # Etiqueta de versión
        ver_short = self.local_version.split(" ")[0] # Muestra solo "1.0.0"
        self.ver_lbl = ctk.CTkLabel(self.sidebar, text=f"v{ver_short}", font=("Consolas", 10), text_color=COLOR_GREEN)
    ...
    def run_update_check(self):
        try:
            ...
            if has_update:
                # Usar 'after' para volver al hilo principal de forma segura
                self.after(0, lambda: self.show_update_notification(updater, msg))
            else:
                print(f"[UPDATER] Sistema actualizado: {msg}")
        ...
        if filepath:
            ...
            def thread_scan():
                ...
                if is_threat:
                    ...
                    if hasattr(self, 'log_box'):
                         # Usar after para evitar otro error de threads aquí
                        self.after(0, lambda: self.log_message(f"AMENAZA: {reason}"))
